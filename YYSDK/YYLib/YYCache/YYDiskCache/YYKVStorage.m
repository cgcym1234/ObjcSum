...
sqlite3 * dbGetHandleWithPath(const char *dbPath) {
    ...
    if (sqlite3_open(dbPath, &db) != SQLITE_OK) {
        printf("open db error, path:[%s] error:[%s] \n", dbPath,sqlite3_errmsg(db) );
...
int dbExecSql(sqlite3 *db, const char *sql) {
    ...
    int reuslt = 0;
    do {
        reuslt = sqlite3_step(stmt);
        if (reuslt == SQLITE_DONE) {
            break;
        } else if (reuslt == SQLITE_ROW) {
            continue;
        } else {
            goto error;
        }
...
- (BOOL)_dbOpen {
    ...
    _dbHandle = dbGetHandleWithPath(_dbPath.UTF8String);
    if (_dbHandle) {
...
//    return dbExecSql(_dbHandle, sql.UTF8String) == DbScucess;
    ...
    char *error = NULL;
    int result = sqlite3_exec(_dbHandle, sql.UTF8String, NULL, NULL, &error);
    if (error) {
...
- (BOOL)_dbSaveWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (filename.length == 0) {
        sqlite3_bind_blob(stmt, 4, value.bytes, valueLength, 0);
    } else {
    ...
    if (result != SQLITE_DONE) {
        if (_errorLogsEnabled) debugLog(@"%s line:%d sqlite insert error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_dbHandle));
        return NO;
    }
    return YES;
}
...
- (BOOL)_dbUpdateAccessTimeWithKey:(NSString *)key {
    ...
    int result = sqlite3_step(stmt);
    if (result != SQLITE_DONE) {
        if (_errorLogsEnabled) debugLog(@"%s line:%d sqlite update error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_dbHandle));
        return NO;
    }
    return YES;
}
