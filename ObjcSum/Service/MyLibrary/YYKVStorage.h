...
typedef NS_ENUM(NSUInteger, YYKVStorageType) {
    ...
    /// 通过item.filename != nil 来决定存储在文件中还是在sqlite中
...
/**
 用sqlite和文件系统的key-value存储,不是线程安全的,
 initWithPath:type:后会创建一个文件夹,不要用该对象之外的方式修改该文件夹
...
/**
 只保存到sqlite中,如果类型是YYKVStorageTypeFile,那么会返回失败
 Save an item or update the item with 'key' if it already exists.
