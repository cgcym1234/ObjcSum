...
#import "CLLocation+YYExtension.h"
...
@implementation CLLocation (YYExtension)
...
-(CLLocation*)yy_locationEarthFromBaidu
{
    double lat = 0.0;
    double lng = 0.0;
    CLLocation *Mars = [self yy_locationMarsFromBaidu];
...
+ (CLLocationCoordinate2D)yy_locationToGaodeFromBaidu:(CLLocationCoordinate2D)location {
    ...
    double lat = 0.0;
    double lng = 0.0;
    transform_baidu_from_mars(location.latitude, location.longitude, &lat, &lng);
...
+ (CLLocationCoordinate2D)yy_locationToBaiduFromGaode:(CLLocationCoordinate2D)location {
    ...
    double lat = 0.0;
    double lng = 0.0;
    transform_mars_from_baidu(location.latitude, location.longitude, &lat, &lng);
    return CLLocationCoordinate2DMake(lat, lng);
}
