...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<YYKeyboardObserver> observer in _observers) {
            if ([observer respondsToSelector:@selector(yyKeyboardManager:keyboardWithTransition:)]) {
                [observer yyKeyboardManager:self keyboardWithTransition:trans];
            }
        }
    }
